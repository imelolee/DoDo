...
func creatVideo(video *model.FeedVideo, data *model.Video, userId int64) {
	...
	micro := InitMicro()
	userClient := userService.NewUserService("userService", micro.Client())
	likeClient := likeService.NewLikeService("likeService", micro.Client())
	commentClient := commentService.NewCommentService("commentService", micro.Client())
	...
	//获取当前用户是否点赞了该视频
	go func() {
		likeRsp, _ := likeClient.IsFavorite(context.TODO(), &likeService.VideoUserReq{
			VideoId: video.Id,
			UserId:  userId,
		})
